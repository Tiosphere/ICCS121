...
int is_alive(life_board board, int row, int col)
{
    if (is_in_range(board, row, col)) {
        if (board.cells[get_index(board, row, col)] == '*'){return 1;}
    }
    return 0;
}
...
{
    return is_alive(board, row-1, col-1) + is_alive(board, row-1, col) + is_alive(board, row-1, col+1) + is_alive(board, row, col-1) + is_alive(board, row, col+1) + is_alive(board, row+1, col-1) + is_alive(board, row+1, col) + is_alive(board, row+1, col+1);
}
...
{
    for (int row = 0; row < current.num_cols; row++)
    {
        for (int col = 0; col < current.num_cols; col++)
        {
            if (is_alive(current, row, col))
            {
                ...
                if (nbrs < 2 || nbrs > 3){set_dead(next, row, col);}
                else {set_alive(next, row, col);}
            }
            else{
                if (count_live_nbrs(current, row, col) == 3) {set_alive(next, row, col);}
                else{set_dead(next, row, col);}
            }
        }
    }
}
